...
public final class LEDDisplayImpl implements LEDDisplay {
    ...
    public void addLEDDisplayChangeListener(LEDDisplayChangeListener l) {
        this.eventListeners.add(LEDDisplayChangeListener.class, l);

    }

    @Override
    public void removeLEDDisplayChangeListener(LEDDisplayChangeListener l) {
        this.eventListeners.remove(LEDDisplayChangeListener.class, l);
    }
    ...
    private boolean checkXY(int x, int y) {
        if (x < 0 || x >= this.width) {
            if (this.strict) throw new IllegalArgumentException(
    ...
    private static int checkWidth(int width) {
        if (width < 1 || width > MAX_WIDTH) throw new IllegalArgumentException(
    ...
    private static int checkHeight(int height) {
        if (height < 1 || height > MAX_HEIGHT) throw new IllegalArgumentException(
    ...
    private static int checkDelay(int delay) {
        if (delay < 0 || delay > MAX_DELAY) throw new IllegalArgumentException(
